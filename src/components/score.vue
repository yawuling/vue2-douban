<template>
  <div class="score">
    <template v-if="score === 0">
      <span class="no-score">暂无评分</span>
    </template>
    <template v-else>
      <span class="rating-stars">
        <span class="rating-star rating-star-full" v-for="n in this.full"></span>
        <span class="rating-star rating-star-gray" v-for="n in this.gray"></span>
      </span>
      <span class="score-average">{{score}}</span>
    </template>
  </div>
</template>

<script>
export default {
  name: 'score',
  props: ['score'],
  data () {
    return {
      full: 0,
      gray: 0
    }
  },
  created () {
    this.full = Math.round(this.score / 2)
    this.gray = 5 - this.full
  }
}
</script>

<style lang="less">
.score{
  margin-top: 5px;
  color: #aaa;
  font-size: 0;
  text-align: center;
}
.rating-stars{
  margin-right: 1px;
}
.rating-star{
  display: inline-block;
  width: 10px;
  height:10px;
  margin-right: 1px;
  background-size: 10px 10px;
  background-color: transparent;
  background-repeat: no-repeat;
}
.rating-star-full{
  background-image: url("data:image/png;base64,");
}
.rating-star-gray{
  background-image: url("data:image/png;base64,");
}
.score-average, .no-score{
  font-size: 12px;
}
</style>
